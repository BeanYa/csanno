...
- 功能分支 - 从 `main` 创建，用于开发新功能
  - 命名约定：
    ...
    - `release/xxx` - 发布版本
    - `hotfix/xxx` - 紧急修复
  其中 `xxx` 为功能名称或修复内容。合并前请进行代码审查。

### 提交规范

提交信息应清晰描述变更内容。参考已有的提交信息，建议描述本次改动。若改动涉及 issue，请在提交信息中引用 issue。

格式：`<类型> @issue#<编号>: <描述>`

示例：
- `feat @issue#123: 添加拦截器异常处理`
- `fix @issue#123: 修复拦截器调用链崩溃问题`
- `docs: 更新项目约定`（单纯的文档修改可不带 issue 编号）
...
- 每次提交前必须运行 `dotnet test`，确保所有测试通过，无错误。
- 提交修改前，需同步修改 `README.md` 和 `CONTRIBUTING.md`（如有需要），确保目录树和模块内容与项目同步。
